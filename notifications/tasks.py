...
def process_offline_notification(self, notification_id):
    ...
    try:
        ...
        if notification.delivery_attempts >= 3:
            logger.info(f"Notification {notification_id} exceeded max retries, sending email fallback")
            send_notification_email(notification)
            notification.mark_failed("Exceeded max WebSocket retry attempts, sent via email")
            return
        ...
        try:
            ...
            logger.info(f"Notification {notification_id} delivered via WebSocket on retry")
        except Exception as e:
            logger.warning(f"WebSocket delivery failed for notification {notification_id}: {str(e)}")
            # Retry with exponential backoff: 1 min, 5 min, 15 min
            retry_delays = [60, 300, 900]
            retry_delay = retry_delays[min(notification.delivery_attempts - 1, len(retry_delays) - 1)]
            raise self.retry(exc=e, countdown=retry_delay)

    except Notification.DoesNotExist:
        logger.error(f"Notification {notification_id} not found")
    except Exception as e:
        logger.error(f"Error processing offline notification {notification_id}: {str(e)}")
        raise
...
def send_email_digest():
    ...
    for user_data in users_with_unread:
        ...
        try:
            ...
            logger.info(f"Sent digest email to user {user_id} with {notification_count} notifications")
        except Exception as e:
...
def cleanup_old_notifications():
    ...
    for priority, ttl in ttl_config.items():
        ...
        logger.info(f"Deleted {deleted_count} {priority} priority notifications older than {ttl.days} days")
